...
function toggleHint(evt) {
  // toggleHint only for left mousedown
...
function showCell (evt) {
  ...
  // After click to show a marked cell, count amount of mines left again
  setMinesLeft()
  // Remove reveal audio
  checkForWin()
}
...
function markCell (evt) {
  evt.preventDefault()
  // mark cell is only for hidden cells
  if (!evt.target.classList.contains('hidden')) return
  startCountingTime()
...
function getSurroundingCells (row, col) {
  ...
  // Add && (cell.row != row ||  cell.col != col) condition to exclude cell itself
  return board.cells
    .filter(function (cell) {
      return columns.includes(cell.col) && rows.includes(cell.row) && (cell.row != row ||  cell.col != col)
    })
}
...
function setInnerHTML (cell) {
  // setInnerHTML function is only for cell that is not mine
